...
@Data
@Slf4j
public class UseQuery implements UseQueryable {
    ...
    public static UseQuery from(final QuerySchema schema, final Object config) {

        return new UseQuery(schema);
    }

    @Override
    public UseQuery resolve(final Schema.Resolver resolver) {

        if (schema.isAnonymous()) {
            return this;
        } else {
            final QuerySchema resolved = resolver.requireQuerySchema(schema.getQualifiedName());
            if (resolved == schema) {
                return this;
            } else {
                return new UseQuery(resolved);
            }
        }
    }
    ...
    public Instance create(final ValueContext context, final Object value, final Set<Name> expand) {
        ...
        return context.createQuery(this, value, expand);
    }
    ...
    public static Instance deserializeAnyValue(final DataInput in) throws IOException {
        ...
        return QuerySchema.deserialize(in);
    }
