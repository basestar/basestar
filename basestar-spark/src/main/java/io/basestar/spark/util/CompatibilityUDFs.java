...
public class CompatibilityUDFs {
    ...
    public static void register(final SparkSession session) {
        ...
        udfs.register("date_add",
                (UDF3<String, Long, Date, Timestamp>) (unit, amount, value) -> DateUnit.from(unit).add(amount, value),
                DataTypes.TimestampType);

        udfs.register("date_diff",
                (UDF3<String, Date, Date, Long>) (unit, a, b) -> DateUnit.from(unit).between(a, b), DataTypes.LongType);

        udfs.register("date_trunc",
                (UDF2<String, Date, Timestamp>) (unit, value) -> DateUnit.from(unit).trunc(value), DataTypes.TimestampType);

        udfs.register("from_iso8601_date",
                (UDF1<String, java.sql.Date>) str -> ISO8601.toSqlDate(ISO8601.parseDate(str)),
                DataTypes.DateType);

        udfs.register("from_iso8601_timestamp",
                (UDF1<String, Timestamp>) str -> ISO8601.toSqlTimestamp(ISO8601.parseDateTime(str)),
                DataTypes.TimestampType);
    }
