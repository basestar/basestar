...
public class UpdateAction implements Action {
    ...
    public Instance after(final ValueContext valueContext, final Context expressionContext, final Instance before) {
        ...
        if (before == null) {
            ...
            beforeVersion = Instance.getVersion(before);
        ...
        if (before != null && !updatedValuesPresent(before, initial)) {
    ...
    private boolean updatedValuesPresent(Instance before, Map<String, Object> after) {
        return after.entrySet().stream()
                .anyMatch(it ->(it.getValue() == null ^ before.get(it.getKey()) == null)
                        || ((before.get(it.getKey()) != null) && !before.get(it.getKey()).equals(it.getValue())));
    }
