...
public class MaxValidation implements Validation {
    ...
    public Optional<Validation.Validator> fromJsr380(final Use<?> type, final Annotation annotation) {
        ...
        if(Max.class.isAssignableFrom(annotationType)) {
            return Optional.of(fromJsr380((Max)annotation));
        } else if(DecimalMax.class.isAssignableFrom(annotationType)) {
            return Optional.of(fromJsr380((DecimalMax)annotation));
        } else {
            return Optional.empty();
        }
    }

    public static Validator fromJsr380(final DecimalMax annotation) {

        return new Validator(new BigDecimal(annotation.value()), !annotation.inclusive());
    }

    public static Validator fromJsr380(final Max annotation) {
    ...
    public static class Validator implements Validation.Validator {
        ...
        public Annotation toJsr380(final Use<?> type, final Map<String, Object> values) {
            ...
            if(type instanceof UseInteger) {
                return new AnnotationContext<>(Max.class, ImmutableMap.<String, Object>builder().putAll(values)
                        .put("value", value.longValue())
                        .put("inclusive", !exclusive)
                        .build()).annotation();
            } else {
                return new AnnotationContext<>(DecimalMax.class, ImmutableMap.<String, Object>builder().putAll(values)
